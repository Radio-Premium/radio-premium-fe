...
RadioPremium은 라디오 음성 콘텐츠에서 광고 구간을 자동으로 탐지하고 스킵하는 시스템을 구현하는 프로젝트입니다.
...
만약 그럴 수 있다면, 라디오는 훨씬 더 **편리하고 즐거운 매체**가 될 수 있을 거라 생각했습니다.  
...
<p align="center">
  <img src="https://github.com/user-attachments/assets/723b1efd-97e5-4d7e-8d8f-10d897d23e1e" alt="핵심기능움짤" width="200" />
</p>

실제 라디오 음성과 함께 동작을 확인하고 싶으시면 [RadioPremium 서비스 시연 영상](https://youtu.be/AzNy1VcAQwc)을 통해 확인하실 수 있습니다.

해당 영상은 SBS 파워FM에서 광고가 재생되면 광고가 재생되지 않는 채널인 KBS 1FM으로 이동합니다. KBS 1FM에 있는 동안 백그라운드에서 계속 SBS 파워FM으로 모니터링하며 SBS 파워FM에서 광고가 종료되면 기존 채널인 SBS 파워FM으로 이동합니다.

### 광고 감지

청취 중인 라디오 채널에서 광고를 자동으로 감지하고 다른 채널로 전환하는 기능을 제공합니다.

“광고 감지” 기능을 켜거나 끌 수 있으며, 각각의 동작 방식은 다음과 같습니다.
| 설정 | 설명 | 동작 |
| ---- | ------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| ON | 현재 듣고 있는 채널에서 광고 여부를 감지합니다. | 광고가 재생되면, 설정한 채널 혹은 광고 없는 랜덤 채널로 자동 이동합니다. |
| OFF | 현재 듣고 있는 채널에서 광고가 재생되어도 광고를 감지하지 않습니다. | 광고를 감지하지 않기 때문에 채널이 자동 이동 되지 않습니다. |

### 기존 채널로 이동

광고 감지로 인해 다른 채널로 전환된 경우, 광고가 끝난 뒤 원래 듣던 채널로 다시 돌아갈지 여부를 설정하는 기능을 제공합니다.

“기존 채널로 이동” 기능을 켜거나 끌 수 있으며, 각각의 동작 방식은 다음과 같습니다.

| 설정 | 설명                                                             | 동작                                                     |
| ---- | ---------------------------------------------------------------- | -------------------------------------------------------- |
| ON   | 기존 채널의 광고 종료 여부를 감지합니다.                         | 광고가 끝나면 원래 듣고 있던 채널로 자동 복귀합니다.     |
| OFF  | 이전 채널에서 광고가 종료되더라도 원래 채널로 돌아가지 않습니다. | 광고가 감지되어 자동 이동된 채널에 계속 머무르게 됩니다. |
...
<p align="center">
  <img src="https://github.com/user-attachments/assets/d86440d5-b36f-491f-bdda-d08999e9d68c" alt="메인움짤" width="200" />
</p>
...
- 즐겨찾는 채널: 사용자가 별도로 등록한 채널 목록
- 일반 채널: 아직 즐겨찾기로 등록되지 않은 모든 채널 목록

각 채널의 이름과 로고를 확인할 수 있으며, 원하는 채널을 클릭하면 즉시 청취를 시작할 수 있습니다.

### 즐겨찾기

자주 듣는 채널을 따로 모아 관리할 수 있도록, 즐겨찾기 기능을 제공합니다.  
별 아이콘을 클릭해 채널을 즐겨찾기에 등록하거나 해제할 수 있습니다.

| 기능 | 설명                                                             | 동작                                                     |
| ---- | ---------------------------------------------------------------- | -------------------------------------------------------- |
| 추가   | 일반 채널 목록에서 별 아이콘을 클릭합니다.                         | 해당 채널이 "즐겨찾는 채널" 목록으로 이동됩니다.     |
| 제거  | 즐겨찾는 채널 목록에서 별 아이콘을 클릭합니다. | 해당 채널이 즐겨찾기에서 해제되고 "일반 채널" 목록으로 이동됩니다. |
| 정렬  | 즐겨찾는 채널 목록 내 순서를 변경합니다. | 드래그앤드롭으로 원하는 순서로 자유롭게 배치할 수 있습니다. |
...
<p align="center">
  <img src=https://github.com/user-attachments/assets/94e05b91-cf42-4487-adf1-5481fd232e9e alt="검색움짤" width="200" />
</p>

</details>

원하는 라디오 채널을 빠르게 찾을 수 있도록 실시간 키워드 기반 검색 기능을 제공합니다.  
검색창에 키워드를 입력하면, 채널 이름에 해당 키워드를 포함하는 채널 목록이 즉시 표시됩니다.  
- 대소문자를 구분하지 않고 검색할 수 있으며,
- 검색 결과에서 바로 채널을 청취하거나 즐겨찾기에 추가할 수 있습니다.

| 기능 | 설명                                                             | 동작                                                     |
| ---- | ---------------------------------------------------------------- | -------------------------------------------------------- |
| 실시간 검색   | 검색어 입력과 동시에 검색된 채널이 화면에 표시됩니다.                        | 키워드를 포함한 채널만 필터링되어 리스트에 표시됩니다.     |
| 즐겨찾기 토글  | 별 아이콘을 클릭합니다. | 즐겨찾기 상태가 변경되어 즐겨찾기 추가/해제됩니다. |
| 안내 메시지  | 검색어가 없거나, 결과가 존재하지 않을 경우 안내 메시지를 제공합니다. | 검색어 없음 → “원하는 키워드를 입력해주세요” / 결과 없음 → “검색 결과가 없습니다” |
...
<p align="center">
  <img src=https://github.com/user-attachments/assets/2d76b7d6-4d4c-4d1b-8b3f-2532698328b0 alt="설정움짤" width="200" />
</p>
  
</details>

라디오 사용 경험을 사용자 취향에 맞게 설정할 수 있는 기능입니다.  
현재 제공되는 설정 항목은 아래와 같으며, 각 항목은 자유롭게 켜고 끌 수 있습니다.

| 기능 | 설명                                                             | 동작                                                     |
| ---- | ---------------------------------------------------------------- | -------------------------------------------------------- |
| 광고 감지   | 현재 듣고 있는 채널에서 광고 여부를 자동으로 탐지합니다.                        | 광고가 감지되면 다른 채널로 자동 전환됩니다.     |
| 기존 채널로 이동  | 광고로 인해 이동된 채널에서, 기존 채널의 광고 종료 여부를 감지합니다. | 광고가 종료되면 원래 듣고 있던 채널로 자동 복귀됩니다.(“광고 감지”가 켜져 있어야 작동) |
| 광고 감지 시 이동할 채널  | 광고 감지 시 자동으로 이동할 채널을 직접 지정할 수 있습니다. | 광고 감지될 경우 설정된 채널로 이동되며, 미지정 시 광고가 없는 채널 중 무작위로 이동합니다.("광고 감지"가 켜져 있어야 표시) |
...
또한 `useStore()`만으로 필요한 상태에 간단히 접근할 수 있고,
...
이처럼 정형 데이터 처리에 적합하면서도 Supabase 실시간 기능과의 연계가 자연스러운 PostgreSQL가 프로젝트에 적합한 데이터베이스라고 판단했습니다.
...
이를 위해 음성을 텍스트로 변환해주는 Whisper 모델(이하 Whisper)을 활용하여, 사용자가 듣고 있는 라디오에서 나오는 음성을 텍스트로 변환하고, 해당 문장에 광고 관련 키워드가 포함되어 있는지를 분석합니다.
...
학습시킨 모델을 사용하여 전달받은 결과에는 광고 여부와 신뢰도 값이 포함되는데 이 신뢰도가 낮은 경우는 광고임을 확신할 수 없고, 실제 광고인데도 감지되지 않는 문제가 발생할 수 있습니다.
...
위 로직을 통해 백엔드는 광고가 최초로 감지된 시점에만 프론트로 채널 전환 신호를 보내며, 이미 광고가 감지된 채널이라면 추가로 감지된 광고는 무시하기 때문에 **불필요한 채널 전환을 방지**할 수 있습니다.

---

### 1-4. 채널 전환

백엔드에서 `socket.emit("radioText", { isAd: true })` 메시지를 전송하면,
...
광고 감지로 채널이 이동된 후, 백엔드에서 기존 채널의 광고 여부를 모니터링하는 과정에 광고가 종료되었다고 판단되면 다음과 같은 로직을 수행합니다.
...
백엔드에서 기존 채널의 광고가 종료되었다고 판단하여 `socket.emit("radioText", { isAd: false })` 메시지를 전송하면,
프론트는 해당 메시지를 받아 기존 채널의 광고가 종료되었음을 인지하고 **기존 채널로 복귀**합니다.

이때 기존 채널로 복귀하면 이전 채널 정보(`prevChannelId`)를 초기화하고 채널 이동 여부(`setIsChannelChanged`)를 `false`로 변경하여
...
처음에는 라디오 음성에서 텍스트를 추출한 후, 그 안에 **광고 관련 키워드가 포함되어 있는지를 기준으로 광고 여부를 판단**했습니다. 예를 들어, “주택청약”, “즉시할인”, "한국농어촌공사" 같은 단어가 들어가 있으면 광고일 가능성이 높다고 판단하는 방식이었습니다.

하지만 음성을 텍스트로 변환해주는 Whisper는 **같은 말도 매번 다르게 인식하는 경우가 많았고**, 광고 문구도 다양한 표현으로 바뀌기 때문에 **단순한 키워드 탐지 방식만으로는 광고를 정확히 구분하기 어려웠습니다**. 이 한계를 극복하기 위해, 저희는 머신러닝을 도입해 텍스트 자체를 분석하고 **자동으로 광고 여부를 분류하는 방법**을 시도하게 되었습니다.

---

### 3-1. 로지스틱 회귀(Logistic Regression) 모델 도입

처음에는 **로지스틱 회귀라는 간단한 분류 모델**을 사용했습니다. 이 모델은 특정 단어나 특징이 있으면 광고일 확률이 높다고 계산하는 방식으로 작동합니다. 빠르고 직관적이지만, **문장의 흐름이나 다양한 표현 방식**을 잘 반영하지는 못합니다. 실제 실험에서도 광고 탐지 정확도(특히 광고를 놓치지 않는 비율, 즉 재현율)가 **50~65% 수준에 머물렀고**, 광고임에도 광고라고 판단되지 않는 한계가 존재했습니다.

---

### 3-2. XGBoost 모델로 전환

더 정확한 탐지를 위해 저희는 **XGBoost라는 비선형 트리 기반 머신러닝 모델**로 전환했습니다. 이 모델은 단어 하나 하나뿐만 아니라, **여러 단어가 어떤 식으로 조합되고, 문장에서 어떤 흐름으로 사용되는지까지 고려**할 수 있습니다. 이에 따라 Whisper가 같은 말이어도 다른 텍스트로 추출되는 문제에 대응할 수 있습니다. 예를 들어 “주택청악”, "즉씨할인"처럼 텍스트를 정확하게 추출하지 못할 수 있어도 선형 모델보다 문맥, 표현, 패턴이 다양하고 복잡할 가능성에 적합한 비선형 트리 모델을 사용하여 정확도를 높일 수 있었습니다.
...
하지만 간헐적으로, **광고가 감지되었음에도 사용자에게 광고 감지 신호가 전달되지 않는 문제**가 발생했습니다.
...
## 2. 광고 감지 후 채널 이동 시, 감지 기준 미분리로 인한 중복 전환 문제

### 문제 상황

사용자 1이 A 채널을 듣고 있을 때 광고가 감지되면, 시스템은 자동으로 광고가 없는 다른 채널 (B)로 이동시킵니다.

하지만 이 과정에서 다음과 같은 문제가 발생했습니다.

> 만약, 사용자 2가 현재 C 채널을 듣고 있으면 광고 여부를 제대로 감지하지 못했습니다.
> 즉, 사용자 1의 A 채널 광고 감지가 사용자 2의 채널 탐지 흐름까지 방해하는 구조가 되었습니다.

### 문제 정의

이 문제의 핵심은 광고 재생 여부를 사용자나 채널 별로 관리하지 않고, 하나의 상태 값으로 처리하면서 발생한 로직 오류입니다.
...
`isAdPlaying`은 **모든 사용자와 채널을 통틀어 하나의 상태로 존재하는 값**입니다.
...
// key: `${userId}:${channelId}`, value: boolean
```

이 구조를 사용하면, 특정 사용자에게 특정 채널에서 광고가 감지되었는지를 개별적으로 기록할 수 있습니다.
예를 들어 사용자 user1이 A 채널에서 광고를 듣고 있는 경우 다음과 같이 기록됩니다.

```jsx
isAdPlaying.set("user1:A", true);
```

이를 통해 사용자 1의 A 채널에서 광고가 감지되더라도, 사용자 2가 B 채널을 듣고 있을 때는 전혀 영향을 받지 않고, 정상적으로 B 채널에서 광고가 재생될 경우 해당 사용자 기준으로만 감지 및 전환 로직이 작동합니다.

#### **2. 구현 과정**

아래는 위 구조를 바탕으로 사용자의 현재 채널 상태에 맞춰 광고 여부를 판단하는 코드입니다.
