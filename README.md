...
<p>
실제 라디오 음성과 함께 동작을 확인하고 싶으시면<br>
<a href="https://youtu.be/AzNy1VcAQwc">🎬 RadioPremium 서비스 시연 영상</a>을 통해 확인하실 수 있습니다.
</p>

<br/>

<p>
SBS 파워FM에서 광고가 재생되면 <strong>광고가 없는</strong> KBS 1FM으로 자동 이동합니다.<br>
KBS 1FM에 있는 동안에도 백그라운드에서 SBS 파워FM을 모니터링하며,<br>
광고가 종료되면 <strong>기존 채널</strong>인 SBS 파워FM으로 자동 복귀합니다.
</p>
...
| 설정 | 설명                            | 동작                                   |
|------|---------------------------------|----------------------------------------|
| ON   | 현재 채널에서 광고를 감지함        | 설정한 채널 또는 무작위 채널로 자동 이동 |
| OFF  | 광고를 감지하지 않음              | 채널 이동 없음                          |
...
| 설정 | 설명                           | 동작                        |
|------|--------------------------------|-----------------------------|
| ON   | 기존 채널의 광고 종료 감지        | 종료 시 원래 채널로 자동 복귀 |
| OFF  | 광고 종료를 감지하지 않음          | 현재 채널에 계속 머무름       |
...
<p>
각 채널의 이름과 로고를 확인하고, 원하는 채널을 클릭하면 바로 청취를 시작할 수 있습니다.
</p>
...
<p>
자주 듣는 채널을 따로 모아 관리할 수 있도록, 즐겨찾기 기능을 제공합니다.<br/>
...
| 기능 | 설명 | 동작 |
| ---- | ---- | ---- |
| 추가 | 일반 채널 목록에서 별 아이콘 클릭 | 즐겨찾는 채널 목록으로 이동 |
| 제거 | 즐겨찾는 채널 목록에서 별 아이콘 클릭 | 즐겨찾기 해제 후 일반 채널 목록으로 이동 |
| 정렬 | 즐겨찾는 채널 순서 변경 | 드래그앤드롭으로 순서 재배치 |
...
<p>
원하는 라디오 채널을 빠르게 찾을 수 있도록, 실시간 검색 기능을 제공합니다.<br/>
...
| 기능         | 설명                                | 동작 결과                                      |
|--------------|-------------------------------------|------------------------------------------------|
| 실시간 검색   | 입력 즉시 키워드 포함 채널만 표시     | 채널 목록이 자동으로 필터링됨                  |
| 즐겨찾기 토글 | 별 아이콘 클릭으로 상태 변경         | 즐겨찾기 추가 또는 해제                         |
| 안내 메시지   | 입력 없음 또는 결과 없음 시 메시지 표시 | 입력 없음: 키워드를 입력해주세요.<br>결과 없음: 관련 채널이 없습니다. |
...
| 기능 | 설명                                                             | 동작                                                     |
| ---- | ---------------------------------------------------------------- | -------------------------------------------------------- |
| 광고 감지   | 현재 채널에서 광고 여부 자동 탐지    | 감지 시 다른 채널로 자동 전환  |
| 기존 채널로 복귀  | 이동된 채널에서 원래 채널의 광고 종료 감지 | 종료 시 자동 복귀<br>(※ 광고 감지 활성 시 작동)    |
| 이동 채널 지정    | 광고 발생 시 이동할 채널 직접 설정  | 설정된 채널로 이동<br>미지정 시 무작위 채널로 전환<br>(※ 광고 감지 활성 시 표시) |
   </td>
  </tr>
</table>
