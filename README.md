...
서비스에 전역으로 관리해야 하는 상태가 다수 존재했고,
프로젝트의 규모와 개발 속도를 고려해 적절한 상태 관리 도구를 고민했습니다.

Redux에 비해 구조가 단순하고, 상태 관리를 위한 코드 양이 적어 빠르게 개발 가능하며
사용자가 많은 라이브러리이면서 꾸준히 유지보수되는 상태 관리 도구를 검토하였습니다.

이런 조건을 충족하면서 사용 방식이 간단하고, 코드 양이 부담스럽지 않은 Zustand가 가장 잘 맞는 선택이라고 판단했습니다.
