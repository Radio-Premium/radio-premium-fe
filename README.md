...
해당 영상은 SBS 파워FM에서 광고가 재생되면 **광고가 재생되지 않는** 채널인 KBS 1FM으로 이동합니다. <br> 
KBS 1FM에 있는 동안 백그라운드에서 계속 SBS 파워FM으로 모니터링하며 SBS 파워FM에서 광고가 종료되면 **기존 채널인** SBS 파워FM으로 이동합니다.
...
| OFF | 현재 듣고 있는 채널에서 광고가 재생되어도<br> 광고를 감지하지 않습니다. | 광고를 감지하지 않기 때문에 채널이 자동 이동 되지 않습니다. |
...
| OFF  | 이전 채널에서 광고가 종료되더라도<br> 원래 채널로 돌아가지 않습니다. | 광고가 감지되어 자동 이동된 채널에 계속 머무르게 됩니다. |
...
| 제거  | 즐겨찾는 채널 목록에서 별 아이콘을 클릭합니다. | 해당 채널이 즐겨찾기에서 해제되고 "일반 채널" 목록으로 <br> 이동됩니다. |
...
| 안내 메시지  | 검색어가 없거나, 결과가 존재하지 않을 경우 안내 메시지를 제공합니다. | 검색어 없음 → “원하는 키워드를 입력해주세요” <br> 결과 없음 → “검색 결과가 없습니다” |
...
이 구조에서는 어떤 사용자가 어느 채널에서 광고를 듣고 있는지를 알 수 없습니다. <br>
즉, 광고가 감지되면 `isAdPlaying = true`로 하나의 상태로 설정되고, 이후 다른 사용자 또는 채널에서 광고가 감지되어도 <br>
